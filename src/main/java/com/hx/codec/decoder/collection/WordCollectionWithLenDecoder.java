 ...
 */
public class WordCollectionWithLenDecoder extends AbstractDecoder<Collection<Integer>> {

    private WordDecoder decoder;
    private ByteType lenByteType;

    public WordCollectionWithLenDecoder(ByteOrder byteOrder, ByteType lenByteType) {
        super(byteOrder);
        this.decoder = new WordDecoder(byteOrder);
        this.lenByteType = lenByteType;
    }

    public WordCollectionWithLenDecoder(ByteType lenByteType) {
        this(DEFAULT_BYTE_ORDER, lenByteType);
    }

    public WordCollectionWithLenDecoder() {
